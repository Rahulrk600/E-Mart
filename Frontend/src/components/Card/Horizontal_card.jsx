        ...
        <div className=' container mx-auto px-4 my-6 relative bg-pink-100'>
            {
                ...
                (
                    <h2 className='text-xl sm:text-2xl text-black font-semibold py-4'>{heading}</h2>
                )
            }
            <div className='flex items-center gap-4 sm:gap-6 overflow-scroll scrollbar-none transition-all ' ref={scrollElement}>
            <button onClick={scrollLeft} className='bg-white text-black shadow-md rounded-full absolute left-0 hidden sm:block'><FaAngleLeft/></button>
            <button onClick={scrollRight}  className='bg-white text-black shadow-md rounded-full absolute right-0 hidden sm:block'><FaAngleRight/></button>
                {
                   loading ?(
